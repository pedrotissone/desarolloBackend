...
const eliminar = async () => {
    alert("te voy a eliminar")
}

const restar = async () => {
    alert("voy a restar 1 a la cantidad del producto")
}
...
const sumar = async (cid, pid) => {
    ...
    if(respuesta.ok) {
        ...
        listQuantity.innerHTML = `<li id="cartQuantity-${cid}-${pid}">Cantidad: ${resultado} <button id="cartButtonRestar" onclick="restar()">-</button> <button id="cartButtonSumar" onclick="sumar('${cid}', '${pid}')">+</button> </li>`        
    } else {
